...
void UPlayerHitByPoliceCarComponent::_knockBack(float _elapsedTime)
{
	// デバッグ用
	...
	{
		// 弾き飛ばす前の始点
		FVector  _initialKnockbackLocation = FVector(_playerActor->GetActorLocation().X, _policeCarActor->GetActorLocation().Y + _offsetPoliceCarPos.Y, _playerActor->GetActorLocation().Z);
		// ノックバック先の位置を計算
		FVector _knockbackTargetLocation = _initialKnockbackLocation + (_knockbackDirection * _knockBackStrength);
		// 指定した時間で徐々に移動するように移動量を計算
		float _alpha = FMath::Clamp(_elapsedTime / _knockbackDuration, 0.0f, 1.0f);		// ノックバックの経過時間を正規化
		float _easeAlpha = FMath::InterpEaseOut(0.0f, 1.0f, _alpha, _knockbackEase);	// 始めは大きく、後半はゆっくり減小するように補間
		FVector KnockbackAmount = FMath::Lerp(_initialKnockbackLocation, _knockbackTargetLocation, _easeAlpha);	// 開始から終了の位置を指定した割合で補間
		// 決まった方向に指定した強さでノックバックさせる
		_playerActor->SetActorLocation(KnockbackAmount);
	}
}
